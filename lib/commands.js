...
async function selectChair(
  ...
  try {
    ...
    do {
      ...
      const numberOfChairs = (await searchAllElements(page, rowSelector))
        .length;
      seat = await selectOneElement(selectRow, chairSelector, chair);
      if (flagTaken && chair < numberOfChairs - 1) {
        console.log(
          `Стул ${chair + 1} в ${row + 1} ряду занят, проверяю ${
            chair + 2
          } стул`
        );
        chair++;
      ...
      takenStatus = await seat.evaluate(
        (el, taken) => el.classList.contains(taken),
        takenSelector
      );
